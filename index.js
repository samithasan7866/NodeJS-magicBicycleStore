...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === "/") {
    ...
    let eachBicycle = await fs.readFile(
      `${__dirname}/views/partials/bicycle.html`,
      "utf-8"
    );

    const allTheBicycles = bicycles
      .map((bicycle) => replaceTemplate(eachBicycle, bicycle))
      .join(" ");
  ...
  // IMAGE
    ...
    let image = await fs.readFile(
      `${__dirname}/public/images/${req.url.slice(1)}`
    );
...
function replaceTemplate(html, bicycle) {
  ...
  html = html.replace(
    /<%DISCOUNT%>/g,
    hasDiscount
      ? `<div class="discount__rate"><p>${discount}% Off</p></div>`
      : ""
  );
